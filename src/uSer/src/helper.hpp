...
std::string SerializationMethodToStr(serialization_method_t method) {
  return method == serialization_method_t::copy ? "COPY" 
        : method == serialization_method_t::sg ? "SG" : 
        method == serialization_method_t::zc? "ZC" : "None";
}
...
public:
  ...
  static inline float GetRandrange(float min, float max) {
    float scale = rand() / (float)RAND_MAX; 
    return min + scale * (max - min);       
  }

static inline double GetRandrange(double min, double max) {
