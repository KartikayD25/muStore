...
struct Conn {
  ...
  Conn(enum ConnProto proto, int sock_) : proto_(proto), sock_(sock_) {
    if (sock_ < 0) {
      common::HandleSyscallError("socket");
    }
  }
  ...
  size_t Writev(const struct iovec iov[], const int iov_len) {
    std::cout << " Hello from writev" << std::endl;
  ...
  size_t sendmsg_SG(const msghdr *__msg) { // Do we need to overload the
                                           // method??
    ssize_t bytes = sendmsg(sock_, __msg, 0);
...
private:
  ...
  //         if (serr->ee_errno == 0 && serr->ee_origin ==
...
struct TCPClient : TCPConn {
  ...
  bool Connect() {
    ...
    int sendbuf = 104857600; // 100MB buffer
    setsockopt(sock_, SOL_SOCKET, SO_SNDBUF, &sendbuf, sizeof(sendbuf));

    int flag = 1;
    setsockopt(sock_, IPPROTO_TCP, TCP_NODELAY, &flag, sizeof(flag));

    int cork = 1;
    setsockopt(sock_, IPPROTO_TCP, TCP_CORK, &cork, sizeof(cork));
    return is_active_ = true;
  }
