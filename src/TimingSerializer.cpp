...
TimingSerializer::TimingSerializer(std::unique_ptr<ISerializer> base,
                                   const std::string &csvPath)
    : baseSerializer(std::move(base)) {
  csvFile.open(csvPath, std::ios::app);
  if (!csvFile.tellp()) {
    csvFile << "timestamp,serializer_type,data_size_bytes,duration_ns,cache_"
               "misses,branch_misses,cpu_cycles,max_memory_kb\n";
  }
...
int TimingSerializer::serialize(const Response &msg) {
...
#ifdef MU_SER_TC
    serializerType = "MuSer(Two Copy)";
#elif MU_SER_OC
    serializerType = "MuSer(One Copy)";
#else
    serializerType = "MuSer(Zero Copy)";
#endif
  ...
  logTiming(serializerType, result, duration, cacheMisses, branchMisses,
            cpuCycles, maxMemory);
  return result;
...
void TimingSerializer::logTiming(const std::string &serializerType,
                                 size_t dataSize, long long duration,
                                 long long cacheMisses, long long branchMisses,
                                 long long cpuCycles, long long maxMemory) {
  ...
  csvFile << timestamp << "," << serializerType << "," << dataSize << ","
          << duration << "," << cacheMisses << "," << branchMisses << ","
          << cpuCycles << "," << maxMemory << "\n";
  csvFile.flush();
}
