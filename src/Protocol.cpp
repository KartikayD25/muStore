...
std::unique_ptr<ISerializer> Protocol::serializer =
    std::make_unique<TimingSerializer>(std::make_unique<IOVecSerializer>(),
                                       "serializer_timing.csv");
#endif

#ifdef MU_SER
std::unique_ptr<ISerializer> Protocol::serializer =
    std::make_unique<TimingSerializer>(std::make_unique<MuSer>(),
                                       "serializer_timing.csv");
#endif
...
#ifdef TWO_COPY
std::unique_ptr<ISerializer> Protocol::serializer =
    std::make_unique<TimingSerializer>(std::make_unique<TwoCopySerializer>(),
                                       "serializer_timing.csv");
#endif

#ifdef PROTO_BUF
std::unique_ptr<ISerializer> Protocol::serializer =
    std::make_unique<TimingSerializer>(std::make_unique<Protobuf>(),
                                       "serializer_timing.csv");
#endif

#ifdef FLAT_BUF
std::unique_ptr<ISerializer> Protocol::serializer =
    std::make_unique<TimingSerializer>(std::make_unique<FlatBuffers>(),
                                       "serializer_timing.csv");
#endif
...
#if defined(ZERO_COPY) || defined(MU_SER_ZC)
  LOG_INFO("Setting SO_ZEROCOPY");
